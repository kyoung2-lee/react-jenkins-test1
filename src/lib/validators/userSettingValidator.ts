...
export const requiredSameRowApo = (value: string, allValues: any, _props: unknown, name: string) => {
  if (value) {
    return undefined;
  }
  const regex = /(?<ntfListType>[a-zA-Z]*)\[\]\[(?<index>\d*)\]\[(?<event>[a-zA-Z]*)\]/gm;
  const macth: RegExpExecArray | null = regex.exec(name);
  ...
  if (macth && macth.groups) {
    ...
    if (allValues[ntfListType] && allValues[ntfListType][index]) {
      const fields: { [key: string]: string } = allValues[ntfListType][index];
...
export const requiredSameRowFlt = (value: string, allValues: any, _props: any, name: string) => {
  if (value) {
    return undefined;
  }
  const regex = /(?<ntfListType>[a-zA-Z]*)\[\]\[(?<index>\d*)\]\[(?<event>[a-zA-Z]*)\]/gm;
...
export const duplicationValusSameRowApo = (value: string, allValues: any, _props: any, name: string) => {
  // 入力値がない場合、チェックをしない
  if (!value) {
    return undefined;
  }
  ...
  if (macth && macth.groups) {
    ...
    if (allValues[ntfListType] && allValues[ntfListType][index]) {
      // 条件に応じて必要な項目のみを抽出
      const valusString: string = JSON.stringify(allValues[ntfListType][index]);
      // 対象のFildNameが存在する同一行と全て同じ値があるか確認
      for (let i = 0; i < allValues[ntfListType].length; i++) {
        if (index !== String(i)) {
          const targetValusString: string = JSON.stringify(allValues[ntfListType][i]);
          if (valusString === targetValusString) {
            return SoalaMessage.M50019C;
          }
        }
      }
    }
...
export const duplicationValusSameRowFlt = (value: string, allValues: any, _props: any, name: string) => {
  // 入力値がない場合、チェックをしない
  if (!value) {
    return undefined;
  }
