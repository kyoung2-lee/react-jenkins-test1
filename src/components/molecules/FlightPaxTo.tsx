...
import { FlightPaxToListState } from "../../reducers/flightContentsFlightPaxTo";
import {
  MODAL_PAX_AREA_WIDTH,
...
export default class FlightPaxTo extends React.PureComponent<Props> {
    ...
    return (
      <Wrapper>
        <Header onClick={scrollContentOnClick}>
          <HeaderLeft>
            <HeaderArrDate>{flightPaxTo.myFlightXtaLclDate}</HeaderArrDate>
            <HeaderArrTime>{flightPaxTo.myFlightXtaLclTime}</HeaderArrTime>
          </HeaderLeft>
          <HeaderRight>
            <HeaderApoCd>{flight.lstArrApoCd}</HeaderApoCd>
            <HeaderSpotText>Spot</HeaderSpotText>
            <HeaderSpotNo>{flightPaxTo.myFlightArrSpotNo || FIGURE_SPACE_CHARACTER}</HeaderSpotNo>
          </HeaderRight>
        </Header>
        <MainContentHeader onClick={scrollContentOnClick}>
          <MainContentHeaderShadowing>
            ...
            <TimeGridContainer>
              {flightPaxTo.timeBarIndicators.map((timeBarIndicator, timeCellIndex) => (
                <TimeCell
                  // eslint-disable-next-line react/no-array-index-key
                  key={`time-${timeCellIndex}`}
                  left={flightPaxTo.timeBarOffsetLeft * -1 + MODAL_MAIN_AREA_CELL_WIDTH * timeCellIndex}
                >
                  {timeBarIndicator}
                </TimeCell>
              ))}
          ...
          <DepFltTextComponent>DEP FLT</DepFltTextComponent>
          {flightPaxTo.timeLclIsInBound && <PaxCurrentTimeBall left={currentTimeBarLeft} />}
        </MainContentHeader>
        <MainContent tabIndex={-1} ref={this.mainContentRef} onClick={scrollContentOnClick}>
          {flightPaxTo.ontoData.map((ontoDatum, index) => (
            // eslint-disable-next-line react/no-array-index-key
            <MainContentRow key={`ontoData-${index}`}>
              <CnxInfo>
                <PaxNumberRow>
                  <PaxNumberRowSpan>
                    <AdltAndChldNumber>{ontoDatum.adltAndChlds}</AdltAndChldNumber>
                    {!!ontoDatum.infts && (
                      <>
                        <OperandPlus>+</OperandPlus>
                        <InftNumber>{ontoDatum.infts > 99 ? "XX" : ontoDatum.infts}</InftNumber>
                      </>
                    )}
                  </PaxNumberRowSpan>
                </PaxNumberRow>
                {!ontoDatum.isUnknownXtdLcl ? (
                  <PaxTimeRow lazy={ontoDatum.isLazy}>
                    {ontoDatum.isLazy && "-"}
                    {ontoDatum.durationAbsoluteTime}
                  </PaxTimeRow>
                ) : (
              ...
              <FlightGridContainer>
                ...
                {[...Array(5)].map((_, gridCellIndex) => (
                  <FlightGridCell
                    ...
                    left={flightPaxTo.timeBarOffsetLeft * -1 + MODAL_MAIN_AREA_CELL_WIDTH * gridCellIndex}
                    width={MODAL_MAIN_AREA_CELL_WIDTH}
                  />
                ))}
                <PaxLazyArea key={`paxLazyArea-${ontoDatum.alCd}${ontoDatum.fltNo}`} width={MODAL_MAIN_AREA_CELL_WIDTH} />
                <PaxLineContainer width={ontoDatum.paxLineBarWidth}>
                  <PaxLineBarContainer>
                    <PaxLineBar />
                    <PaxLineShip />
                  </PaxLineBarContainer>
                  <PaxLineMetaContainer className={this.paxLineMetaContainerClassName}>
                    {!ontoDatum.isUnknownXtdLcl ? (
                      <>
                        {ontoDatum.xtdLclDate !== flightPaxTo.myFlightXtaLclDate && <PaxLineShipTag>{ontoDatum.xtdLclDate}</PaxLineShipTag>}
                        <PaxLineStartTime>{ontoDatum.xtdLclTime}</PaxLineStartTime>
                      </>
                    ) : (
              ...
              <FlightInfo>
                <FlightInfoRow>
                  <FlightInfoAlCd>{ontoDatum.alCd}</FlightInfoAlCd>
                  <FlightInfoFltNo>{ontoDatum.fltNo}</FlightInfoFltNo>
                </FlightInfoRow>
                {ontoDatum.isUnknownXtdLcl ? (
                  <div>{FIGURE_SPACE_CHARACTER}</div>
                ) : ontoDatum.depAptCd === flight.lstArrApoCd ? (
                  <GateInfoRow>
                    <GateTextComponent>Gate</GateTextComponent>
                    <GateNo>{ontoDatum.depGateNo || FIGURE_SPACE_CHARACTER}</GateNo>
                  </GateInfoRow>
                ) : (
                  <AlterAptCd>{ontoDatum.depAptCd}</AlterAptCd>
                )}
              </FlightInfo>
            </MainContentRow>
          ))}
          <LastMainContentRow items={flightPaxTo.ontoData.length}>
            <LastFlightGridContainer />
            <LastFlightInfo />
          </LastMainContentRow>
          {flightPaxTo.timeLclIsInBound && (
            <PaxCurrentTimeBar key="paxCurrentTimeBar" left={currentTimeBarLeft} items={flightPaxTo.ontoData.length} />
          )}
        </MainContent>
        <FlightFooter>
...
const HeaderArrDate = styled.div`
  font-size: 16px;
`;

const HeaderArrTime = styled.div`
  font-size: 20px;
  margin-left: 3px;
`;
...
const HeaderSpotText = styled.div`
  font-size: 12px;
  margin-left: 4px;
`;

const HeaderSpotNo = styled.div`
  font-size: 20px;
  margin-left: 2px;
`;
...
const DepFltTextComponent = styled(MainContentHeaderTitle)`
  flex-basis: 100px;
...
const PaxLineStartTime = styled.div`
  font-size: 14px;
`;
...
const GateInfoRow = styled.div`
  display: flex;
  align-items: baseline;
`;

const GateTextComponent = styled.div`
  font-size: 10px;
`;

const GateNo = styled.div`
  margin-left: 2px;
  font-size: 16px;
`;
...
const PaxCurrentTimeBall = styled.div<{ left: number }>`
  position: absolute;
  left: ${(props) => props.left + MODAL_PAX_AREA_WIDTH - (9 - 1) / 2}px;
  top: 14px;
  width: 9px;
  height: 9px;
  border-radius: 50%;
...
const PaxCurrentTimeBar = styled.div<{ left: number; items: number }>`
  position: absolute;
  top: 0;
  left: ${(props) => props.left + MODAL_PAX_AREA_WIDTH}px;
  width: 1px;
