...
const fisFilterModalSelector = (state) => state.fisFilterModal;
const masterSelector = (state) => state.account.master;
const searchFisRowsSelector = (0, toolkit_1.createSelector)([fisSelector, fisFilterModalSelector, masterSelector], (fisFilter, fisFilterModalFilter, masterFilter) => {
...
// 検索条件を元にフィルタリングする
const doFilter = (fisFilter, searchParams, masterFilter) => {
    ...
    if (searchParams.flightNo) {
        fisRows = fisRows
            .filter((val) => {
            if (val.arr && searchParams.casualFlg
            ...
            if (val.dep && searchParams.casualFlg
    ...
    if (searchParams.cnlHideFlg) {
        fisRows = fisRows.filter((val) => !val.isCancel);
    }
    // Airport
    ...
    if (from || to) {
        if (searchParams.dateTimeRadio === "DEP") {
            fisRows = fisRows
                .filter((val) => {
                const xtdLcl = (0, dayjs_1.default)(val.xtdLcl).format("HHmm");
                if (from && to) {
            ...
            fisRows = fisRows
                .filter((val) => {
                const xtaLcl = (0, dayjs_1.default)(val.xtaLcl).format("HHmm");
                if (from && to) {
...
// ソート条件を元に並び順を変更する
const doSort = (fisRows, isSortArrival, isSortAsc) => {
    // Arrival選択時のソート
    if (isSortArrival) {
        // 日付の配列を取得
        ...
        const maxAtas = dates
            .map((date) => {
            const filterfunc = (f) => f.sortArrDate === date && f.sortArrGroupNo === 1 && f.arr && f.arr.ataLcl;
            const max = (0, lodash_1.maxBy)(fisRows.filter(filterfunc).toArray(), (f) => f.sortXtaLcl);
        ...
        const sortedFisRows = fisRows
            ...
            if (aKey < bKey)
                return -1;
            if (a.fis.arrDepCtrl.seq > b.fis.arrDepCtrl.seq)
                return 1;
            return 0;
        });
        if (isSortAsc) {
    ...
    const maxAtds = dates
        .map((date) => {
        const filterfunc = (f) => f.sortDepDate === date && (f.sortDepGroupNo === 1 || f.sortDepGroupNo === 2) && f.dep && f.dep.atdLcl;
        const max = (0, lodash_1.maxBy)(fisRows.filter(filterfunc).toArray(), (f) => f.sortXtdLcl);
...
exports.default = searchFisRowsSelector;
